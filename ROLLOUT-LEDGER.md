...
| `Artic0din/arsenal` (scaffold, TS+npm) | v1.3.0 (@v1) | [#65](https://github.com/Artic0din/arsenal/pull/65) | partial | None — workflow started, all 4 jobs ran. First repo on stable `@v1` (= v1.3.0). | `package.json` lacks `lint`, `typecheck`, `test`, `test:coverage` scripts → core jobs fail with "missing script". Real failures, not startup_failure. Repo-health PR adds scripts. | Stack validation **PASS** on stable `@v1`. |
| `Artic0din/kilowasps` | — | — | — | — | No GitHub remote. | **Blocked on publish.** Run `gh repo create Artic0din/kilowasps --public` + `git push` then scaffold. |
| `Artic0din/SpoolWise` | — | — | — | — | Same Swift `swift test` vs `xcodebuild` gap as Amprage. | **Deferred to v1.4.** No new validation surface vs Amprage #1. Scaffold once `ci-core-swift.yml` supports `project_type: xcode`. |
